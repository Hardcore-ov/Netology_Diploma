...
from ordering_groceries.celery import app as celery_app
from orders.tasks import send_email, do_import
from orders.models import (Customer, Provider, Category,
                           Characteristic, Order, OrderItem,
                           Product, ProductCharacteristic,
                           Catalog, User, CustomerInfo, Shop,
                           PasswordResetToken)
from orders.permissions import (IsAdmin, IsProvider, IsUser,
                                IsCustomer, IsOwner,
                                IsOrderOwner, IsOrderItemOwner,
                                IsCustomerOwner, IsCatalogOwner,
                                IsCatalogReferencedOwner, IsOrderCatalogOwner)
from orders.serializers import (CustomerSerializer, UserSerializer,
...
class ProviderViewSet(ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == "retrieve":
            RetrievePerm = IsAdmin | IsOwner | IsCustomer
            return [RetrievePerm()]
...
class CatalogViewSet(ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in ["retrieve"]:
            RetrievePerm = IsAdmin | IsCustomer | IsCatalogOwner
            return [RetrievePerm()]
        if self.action == "create":
            return [IsProvider()]
        if self.action in ["update", "partial_update"]:
...
class OrderViewSet(ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == "list":
            return [IsAuthenticated()]
        if self.action == "retrieve":
            RetrievePerm = IsAdmin | IsCustomerOwner | IsOrderCatalogOwner | IsOrderOwner
            return [RetrievePerm()]
        if self.action in ["destroy", "update", "partial_update"]:
...
class OrderItemViewSet(ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action == "list":
            return [IsAuthenticated()]
        if self.action == "retrieve":
            RetrievePerm = IsAdmin | IsOrderItemOwner | IsCatalogReferencedOwner
            return [RetrievePerm()]
        if self.action in ["update", "partial_update"]:
            return [IsCatalogReferencedOwner()]
        return []
